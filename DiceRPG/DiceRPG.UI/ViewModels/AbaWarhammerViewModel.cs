...
public class AbaWarhammerViewModel : AbaBaseViewModel
{
    private readonly JsonDataService _dataService;
    private FichaWarhammer? _fichaAtual;
    private string _fichaSelecionada = "";
    private string _novaFichaNome = "";

    public override string NomeAba => "Warhammer";
    public override string Icone => "Sword";

    public ObservableCollection<string> ListaFichas { get; } = new();

    public FichaWarhammer? FichaAtual
    {
        get => _fichaAtual;
    ...
    public AbaWarhammerViewModel(JsonDataService dataService)
    {
    ...
    private async Task CriarNovaFicha()
    {
        await ExecutarComLoading(async () =>
        {
            ...
            if (await _dataService.FichaExiste(NovaFichaNome, "Warhammer"))
            {
                StatusMessage = "Já existe uma ficha com este nome";
                return;
            }

            FichaAtual = new FichaWarhammer(NovaFichaNome);
    ...
    private async Task CarregarFichaSelecionada()
    {
        ...
        await ExecutarComLoading(async () =>
        {
            var fichas = await _dataService.CarregarFichas<FichaWarhammer>("Warhammer");
